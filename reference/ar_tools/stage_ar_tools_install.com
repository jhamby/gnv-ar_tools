...
$!
$! The file pcsi_ar_tool_file_list.txt is read in to get the files other
...
$!   create/dir new_gnu:[usr.share.doc.ar_tools]/prot=o:rwed
...
$ if mode .eqs. "install"
$ then
$    copy gnv_ar_tools_startup.com -
         new_gnu:[vms_bin]gnv$ar_tools_startup.com
$ else
$    file = "new_gnu:[vms_bin]gnv$ar_tools_startup.com"
...
$   endif
$!
$!   If doc.ar_tools] then
$!       source is sys$disk:[] or [.readme_d]
$!       dest is [usr.share.doc.ar_tools]
$   if f$locate(".doc.ar_tools]", tdir) .lt. tdir_len
$   then
$       source = "sys$disk:[]''tname'''ttype'"
...
$           source = "sys$disk:[]$''tname'''ttype'"
$       endif
$       dest = "new_gnu:[usr.share.doc.ar_tools]''tname'''ttype'"
$       if mode .eqs. "install"
$       then
$           if f$search(dest) .eqs. "" then copy 'source' 'dest'
$       else
